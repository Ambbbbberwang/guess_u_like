...
# DRAFT: rec_fit & rec_test

"""

def rec_fit (train, val, metric = 'RMSE', seed = 42,ranks = [10, 15, 20], 
            regParams = [0.005, 0.01, 0.15], maxIters = [10]):
    ...
    elif metric == 'MAP':   # Are we doing MAP or other ranking metric???
        eval_MAP = None     # To Be Added
        ...
        for j, regParam in enumerate(regParams):
            for p, maxIter in enumerate(maxIters):
                ...
                if metric == 'RMSE':   # No need of Top 500
                    ...
                    print('Implement ranking metric with top 500 recommendation.')
